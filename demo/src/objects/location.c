...
static inline location_object_t *
get_obj(const anjay_dm_object_def_t *const *obj_ptr) {
    assert(obj_ptr);
    return AVS_CONTAINER_OF(obj_ptr, location_object_t, def);
}
...
static int resource_read(anjay_t *anjay,
    ...
    location_object_t *obj = get_obj(obj_ptr);
    assert(obj);
    assert(iid == 0);
...
const anjay_dm_object_def_t **location_object_create(void) {
    location_object_t *obj =
            (location_object_t *) avs_calloc(1, sizeof(location_object_t));
    if (!obj) {
        return NULL;
    }
    obj->def = &OBJ_DEF;
...
void location_object_update(anjay_t *anjay,
    ...
    location_object_t *obj = get_obj(def);
    SYNCHRONIZED(GPS_READ_LAST_MTX) {
        if (GPS_READ_LAST.valid
                && GPS_READ_LAST.timestamp != obj->cached_read.timestamp) {
