...
static int get_fw_version(char *out_buf, size_t buf_size, uint8_t area_id) {
    ...
    static const char *fmt = "%" PRIu16 ".%" PRIu16 ".%" PRIu16 "+%" PRIu32;
    // clang-format off
    if (snprintf(out_buf, BOOT_IMG_VER_STRLEN_MAX, fmt,
                 major, minor, ver->revision, ver->build_num) < 0) {
        return -1;
    }
    // clang-format on
    return 0;
...
int tls_session_cache_purge() {
    ...
    int result = nrf_setsockopt(dummy_socket_fd,
                                NRF_SOL_SECURE,
