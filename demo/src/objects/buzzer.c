...
typedef struct buzzer_state_struct {
    bool on_off;
...
typedef struct buzzer_object_struct {
...
static inline buzzer_object_t *
get_obj(const anjay_dm_object_def_t *const *obj_ptr) {
    assert(obj_ptr);
    return AVS_CONTAINER_OF(obj_ptr, buzzer_object_t, def);
}

static int buzzer_enable(buzzer_object_t *obj) {
...
static int buzzer_disable(buzzer_object_t *obj) {
...
static void buzzer_disable_work_handler(struct k_work *work) {
    ...
    buzzer_object_t *obj =
            AVS_CONTAINER_OF(dwork, buzzer_object_t, disable_buzzer_dwork);
    buzzer_disable(obj);
...
static int buzzer_reschedule(buzzer_object_t *obj) {
    bool was_scheduled =
...
static int instance_reset(anjay_t *anjay,
    ...
    buzzer_object_t *obj = get_obj(obj_ptr);
    assert(obj);
    assert(iid == 0);
...
const anjay_dm_object_def_t **buzzer_object_create(void) {
    ...
    buzzer_object_t *obj =
            (buzzer_object_t *) avs_calloc(1, sizeof(buzzer_object_t));
    if (!obj) {
        return NULL;
    }
    obj->def = &OBJ_DEF;
